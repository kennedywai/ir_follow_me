...
public:
    ...
    : it_(nh_)
  {
    ...
    target_xywh_pub_ = nh_.advertise<geometry_msgs::Vector3>("/target_xywh", 1000);
...
//    cv::destroyWindow(OPENCV_WINDOW);
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    face_cascade.detectMultiScale( frame_gray, faces, 1.05, 5); // 35ms
    ...
    if (faces.size() > 0) {
        ...
        //firstBodyFound = 1;
        before_filter_x = (faces[0].x + faces[0].width/2) * 4 - 40;
        before_filter_y = (faces[0].y + faces[0].height/2) * 4 - 30;
    }
    else
    {
        //myFilter(0.0, 0.0);
...
//    printf("%f    %f    %f\n", t, before_filter_x, filter_x);
    ...
    if (1) {
        size_t i = 0;
        Point center(filter_x + 40.0, filter_y + 30);
        ...
        */
        ...
        // publishing
        brray.x = -filter_x; // y: -320 ~ +320
        brray.y = -filter_y; // z: -240 ~ +240
        brray.z = 0.0;

        target_xywh_pub_.publish(brray);
        printf("%f    %f    %f\n", brray.x, brray.y, brray.z);
